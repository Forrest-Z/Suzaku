...
State::State(
    IterationCount max_iters, const std::vector<int64_t> &ranges, int thread_i,
    ...
    : total_iterations_(0), batch_leftover_(0), max_iterations(max_iters),
      ...
      complexity_n_(0), counters(), thread_index(thread_i), threads(n_threads),
...
#pragma GCC diagnostic ignored "-Winvalid-offsetof"
    ...
    static_assert(
        offsetof(State, error_occurred_) <=
    ...
    BenchmarkReporter *display_reporter, BenchmarkReporter *file_reporter)
{
    ...
    for (const BenchmarkInstance &benchmark : benchmarks)
    {
        ...
        for (const auto &Stat : *benchmark.statistics)
            stat_field_width =
                std::max<size_t>(stat_field_width, Stat.name_.size());
    }
...
void ParseCommandLineFlags(int *argc, char **argv)
{
    ...
    for (int i = 1; i < *argc; ++i)
    {
        if (ParseBoolFlag(
            ...
            ParseStringFlag(
                argv[i], "benchmark_filter", &FLAGS_benchmark_filter) ||
            ParseDoubleFlag(
                argv[i], "benchmark_min_time", &FLAGS_benchmark_min_time) ||
            ParseInt32Flag(
                argv[i], "benchmark_repetitions",
                &FLAGS_benchmark_repetitions) ||
            ParseBoolFlag(
                argv[i], "benchmark_report_aggregates_only",
                &FLAGS_benchmark_report_aggregates_only) ||
            ParseBoolFlag(
                argv[i], "benchmark_display_aggregates_only",
                &FLAGS_benchmark_display_aggregates_only) ||
            ParseStringFlag(
                argv[i], "benchmark_format", &FLAGS_benchmark_format) ||
            ParseStringFlag(argv[i], "benchmark_out", &FLAGS_benchmark_out) ||
            ParseStringFlag(
                argv[i], "benchmark_out_format", &FLAGS_benchmark_out_format) ||
            ParseStringFlag(
                argv[i], "benchmark_color", &FLAGS_benchmark_color) ||
            // "color_print" is the deprecated name for "benchmark_color".
            // TODO: Remove this.
            ParseStringFlag(argv[i], "color_print", &FLAGS_benchmark_color) ||
            ParseBoolFlag(
                argv[i], "benchmark_counters_tabular",
...
bool ReportUnrecognizedArguments(int argc, char **argv)
{
    ...
    {
        fprintf(
            stderr, "%s: error: unrecognized command-line flag: %s\n", argv[0],
            argv[i]);
    }
    return argc > 1;
}
